...
class Mario:
    def __init__(self, state_dim, action_dim, save_dir):
        ...
        state = torch.load(r'checkpoints\2022-12-04T20-41-07\mario_net_end.chkpt')
        self.net.load_state_dict(state['state_dict'])

        self.net = self.net.to(device=self.device)

        self.state_exists = 'state' in locals()

        if self.state_exists:
            self.exploration_rate = state['epsilon']
            self.total_episodes = state['episode']
        else:
...
class Mario(Mario):
    ...
    def save_end(self, episode):
        save_path = (
            self.save_dir / "mario_net_end.chkpt"
        )
...
for e in range(mario.total_episodes, mario.total_episodes + episodes):
...
mario.save_end(mario.total_episodes + episodes)
logger.record(episode=e, epsilon=mario.exploration_rate, step=mario.curr_step)
